...
function populateFilters() {
    ...
    if (artistFilter) {
        artistFilter.innerHTML = '<option value="all">Все артисты</option>';
    ...
    if (albumsArtistFilter) {
        albumsArtistFilter.innerHTML = '<option value="all">Все артисты</option>';
        allArtists.forEach(artist => {
            albumsArtistFilter.innerHTML += `<option value="${artist.id}">${artist.name}</option>`;
        });
    }

    const albumFilter = document.getElementById('album-filter');
    if (albumFilter) {
        albumFilter.innerHTML = '<option value="all">Все альбомы</option><option value="no-album">Без альбома</option>';
        allAlbums.forEach(album => {
            const artist = allArtists.find(a => a.id === album.artist_id);
            albumFilter.innerHTML += `<option value="${album.id}">${album.title} — ${artist?.name || ''}</option>`;
        });
    }
}
...
function getScoreClass(score) {
    ...
    return `score-${Math.min(10, Math.max(1, rounded))}`;
}
...
function updateStats() {
    ...
    if (currentTab === 'tracks') {
        ...
        statsBar.innerHTML = `
            <div class="stat"><span>${total}</span><small>треков</small></div>
            <div class="stat"><span style="color: ${getScoreColor(parseFloat(avgScore))}">${avgScore}</span><small>средняя</small></div>
            <div class="stat"><span style="color: ${getScoreColor(parseFloat(topScore))}">${topScore}</span><small>лучшая</small></div>
        `;
    } else if (currentTab === 'albums') {
        statsBar.innerHTML = `
            <div class="stat"><span>${allAlbums.length}</span><small>альбомов</small></div>
            <div class="stat"><span>${allTracks.length}</span><small>треков</small></div>
            <div class="stat"><span>${allArtists.length}</span><small>артистов</small></div>
        `;
...
function updateReorderButton() {
    ...
    if (currentTab === 'tracks') {
        sortSelect = document.getElementById('sort-select')?.value;
    } else if (currentTab === 'albums') {
        sortSelect = document.getElementById('albums-sort-select')?.value;
    } else {
        sortSelect = document.getElementById('artists-sort-select')?.value;
    }
...
function applyTracksFilters() {
    ...
    if (artistFilter !== 'all') {
        filtered = filtered.filter(t => t.artist_id === artistFilter);
    }
...
function applyAlbumsFilters() {
    ...
    if (artistFilter !== 'all') {
        filtered = filtered.filter(a => a.artist_id === artistFilter);
    }

    // Добавляем вычисленную среднюю оценку
    filtered = filtered.map(album => ({
        ...album,
        avgScore: getAlbumAvgScore(album)
    }));
...
function applyArtistsFilters() {
    ...
    let filtered = allArtists.map(artist => {
        ...
        const albums = allAlbums.filter(a => a.artist_id === artist.id);
        const avgScore = tracks.length > 0 
            ? tracks.reduce((sum, t) => sum + t.total_score, 0) / tracks.length 
            : 0;
        return { ...artist, avgScore, trackCount: tracks.length, albumCount: albums.length };
    });
    ...
    switch (sortBy) {
        ...
        case 'tracks':
            ...
            break;
        case 'albums':
            filtered.sort((a, b) => b.albumCount - a.albumCount);
            break;
        default:
        ...
        return `
            <div class="track-card" data-id="${track.id}">
                ...
                <div class="track-info">
                    ...
                    <div class="track-artist">${artist?.name || track.artist}</div>
                </div>
...
function renderAlbums(albums) {
    ...
    list.innerHTML = albums.map(album => {
        const albumTracks = allTracks.filter(t => t.album_id === album.id);
        const avgScore = album.avgScore !== undefined ? album.avgScore : getAlbumAvgScore(album);
        const scoreClass = albumTracks.length > 0 ? getScoreClass(avgScore) : '';
        ...
        if (viewMode === 'list') {
            return `
                <div class="album-card" data-id="${album.id}">
                    ...
                    <div class="album-score ${scoreClass}">${avgScore > 0 ? avgScore.toFixed(1) : '—'}</div>
                </div>
            `;
        } else {
            return `
                <div class="album-card" data-id="${album.id}">
                    ...
                    <div class="album-title">${album.title}</div>
                    <div class="album-artist">${artist?.name || ''}</div>
                    <div class="album-score ${scoreClass}">${avgScore > 0 ? avgScore.toFixed(1) : '—'} (${albumTracks.length})</div>
                </div>
            `;
        }
    }).join('');
...
function renderArtists(artists) {
    ...
    list.innerHTML = artists.map(artist => {
        const avgScore = artist.avgScore || 0;
        const scoreClass = artist.trackCount > 0 ? getScoreClass(avgScore) : '';
        const cover = artist.image_url ? `<img src="${artist.image_url}" alt="">` : '';

        return `
            <div class="artist-card" data-id="${artist.id}">
                <div class="artist-avatar">
                    ${cover || artist.name.charAt(0).toUpperCase()}
                </div>
                <div class="artist-info">
                    <div class="artist-name">${artist.name}</div>
                    <div class="artist-stats">${artist.albumCount} альбомов • ${artist.trackCount} треков</div>
                </div>
                <div class="track-score ${scoreClass}">${avgScore > 0 ? avgScore.toFixed(1) : '—'}</div>
            </div>
        `;
    }).join('');
    ...
    list.querySelectorAll('.artist-card').forEach(card => {
        card.addEventListener('click', () => {
            window.location.href = `artist.html?id=${card.dataset.id}`;
        });
    });
}
...
function openReorderModal() {
    ...
    if (currentTab === 'tracks') {
        ...
        items = [...allTracks].sort((a, b) => {
            ...
            id: t.id,
            title: t.title,
            subtitle: allArtists.find(a => a.id === t.artist_id)?.name || t.artist,
            score: t.total_score,
        ...
        items = [...allAlbums].map(a => ({
            ...
            id: a.id,
            title: a.title,
            subtitle: allArtists.find(ar => ar.id === a.artist_id)?.name || '',
            score: a.avgScore,
            cover: a.image_url
        }));
    } else {
        ...
        items = allArtists.map(a => {
            const tracks = allTracks.filter(t => t.artist_id === a.id);
            const avgScore = tracks.length > 0 
                ? tracks.reduce((sum, t) => sum + t.total_score, 0) / tracks.length 
                : 0;
            return { ...a, avgScore };
    ...
    list.innerHTML = items.map((item, index) => `
        <div class="reorder-item" data-id="${item.id}" data-index="${index}">
            <div class="reorder-handle">☰</div>
            <div class="reorder-position">${index + 1}</div>
            <div class="reorder-cover">${item.cover ? `<img src="${item.cover}" alt="">` : '🎵'}</div>
            <div class="reorder-info">
                <div class="reorder-title">${item.title}</div>
                <div class="reorder-subtitle">${item.subtitle}</div>
            </div>
...
function updatePositions() {
    ...
    items.forEach((item, index) => {
        item.querySelector('.reorder-position').textContent = index + 1;
...
async function saveOrder() {
    ...
    items.forEach((item, index) => {
        updates.push({
            id: item.dataset.id,
            custom_order: maxOrder - index
        });
    });

    const table = reorderType;
...
document.getElementById('reorder-modal-close').addEventListener('click', () => {
    document.getElementById('reorder-modal').classList.add('hidden');
});
